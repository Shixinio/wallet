...
var BRS = (function(BRS, $, undefined) {
    BRS.automaticallyCheckRecipient = function() {
        ...
        $recipientFields.on("checkRecipient", function() {
            ...
            if (value && value != "COWBOY-____-____-____-_____") {
    ...
    BRS.sendMoneyCalculateTotal = function(element) {
        ...
        $(element).closest(".modal").find(".total_amount_ordinary").html(BRS.formatAmount(BRS.convertToNQT(amount + fee)) + " COWBOY");
    };
            ...
            if (response.publicKey) {
                ...
                if (response.errorCode) {
                    if (response.errorCode === 4) {
                        callback({
                            "type": "danger",
                            "message": $.t("recipient_malformed") + (!/^(COWBOY\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
                            "account": null
                        });
    ...
    BRS.checkRecipient = function(account, modal) {
        ...
        if (/^(COWBOY\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
