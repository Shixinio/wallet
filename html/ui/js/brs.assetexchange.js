...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.forms.addAssetBookmark = function($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^COWBOY\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^COWBOY\-/i.test(data.id)) {
    ...
    $("#asset_order_modal").on("show.bs.modal", function(e) {
        ...
        $("#asset_order_total").html(totalNXT + " COWBOY");
        $("#asset_order_fee_paid").html(BRS.formatAmount(feeNQT) + " COWBOY");
