...
public final class GetAccount extends APIServlet.JsonRequestHandler {
  ...
  GetAccount(ParameterService parameterService, AccountService accountService, BlockService blockService) {
  ...
  JsonElement processRequest(HttpServletRequest req) throws BurstException {
    ...
    response.addProperty("capacity", this.blockService.getAccountCapacity(account));

    response.addProperty("bestMortgage",this.blockService.getBestMortgage(account));

    response.addProperty("mortgageState",this.blockService.getMortgageState(account)+"%");
